...
module.exports = {
	...
	int_to_ip: function (ip_int) {
		return ((ip_int >>> 24) + '.' + (ip_int >> 16 & 255) + '.' + (ip_int >> 8 & 255) + '.' + (ip_int & 255));
	},

	ip_to_int: function (ip_address) {
		var ip = ip_address.match(/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/);
		if (ip) {
	...
	is_ip: function (ip_string) {
		var ip_pattern = /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/;
		return ip_pattern.test(ip_string);
	},

	is_subnet: function (subnet) {
		var subnet_pattern = /((\d){1,3}\.){3}(\d){1,3}\/(\d){1,3}/;
		return subnet_pattern.test(subnet);
	},

	ip_mask: function (mask_size) {
		if (this.is_ip(mask_size)) mask_size = this.ip_to_int(mask_size);
		return -1 << (32 - mask_size);
	},

	test_in_subnet: function (input_ip, subnet) {
		if (!(this.is_ip(input_ip) && this.is_subnet(subnet))) {
			return false;
		}
		input_ip = this.ip_to_int(input_ip);
		...
		// https://www.regextester.com/94015
		// For example, 123.123.48.0/22 represents (123<<24)+(123<<16)+(48<<8)+0 (=2071670784; 
		// this might be a negative number) as a 32 bit numeric IP address, and -1<<(32-22) = -1024 as a mask. 
		// With this, and likewise, your test IP address converted to a number.

		var components = subnet.split('/');
		var test_ip = this.ip_to_int(components[0]);
		var netmask = this.ip_mask(components[1]);
		var test = (input_ip & netmask) == test_ip;
		return test;
	},

	testService: function (host, timeout, cb) {
