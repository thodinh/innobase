...
module.exports = class extends BaseEvent {
  ...
  __init(callback) {
    ...
    if (this.config.get("log_dir")) {
      try {
        ...
        var msg =
          "FATAL ERROR: Log directory could not be created: " +
    ...
    this.logger = new Logger(
      Path.join(
        this.config.get("log_dir") || ".",
        this.config.get("log_filename") || "event.log"
      ),
      this.config.get("log_columns") || [
        "hires_epoch",
        "date",
        "hostname",
        "pid",
    ...
    if (
      ...
      this.logger.set("echoer", function(line, cols, args) {
        if (("" + args.component).match(re) || ("" + args.category).match(re)) {
          if (self.color)
            process.stdout.write(self.logger.colorize(cols) + "\n");
    ...
    if (!this.debug && !this.foreground) {
      ...
      if (!process.env.__daemon) {
        ...
        this.logDebug(
          2,
          "Spawning background daemon process (PID " +
            process.pid +
            " will exit)",
          process.argv
        );
    ...
    if (!this.debug) {
      ...
      if (this.config.get("log_crashes")) {
        require("uncatch").on("uncaughtException", function(err) {
          fs.appendFileSync(
            Path.join(self.config.get("log_dir"), "crash.log"),
            new Date().toString() +
              " - " +
              os.hostname() +
              " - PID " +
    ...
    if (this.config.get("pid_file")) {
      ...
      if (pid) {
        this.logDebug(
          1,
          "WARNING: An old PID File was found: " + pid_file + ": " + pid
        );

        var ping = false;
        try {
          ping = process.kill(pid, 0);
        } catch (e) {}
        ...
        if (ping) {
          var msg =
            ...
            " from " +
            pid_file +
            " is still alive and running.  Aborting startup.";
          ...
          this.logDebug(
            ...
            "Old process " +
              pid +
              " is apparently dead, so the PID file will be replaced: " +
              pid_file
          );
        }
      }
      ...
      try {
        ...
        var msg =
          "FATAL ERROR: PID file could not be created: " + pid_file + ": " + e;
        this.logger.set("sync", true);
        this.logDebug(1, msg);
        process.exit(1);
      }
      ...
      try {
        ...
        this.logDebug(
          9,
          "Confirmed PID File contents: " + pid_file + ": " + pid
        );
      } catch (e) {
        var msg =
          "FATAL ERROR: PID file could not be read: " + pid_file + ": " + e;
        this.logger.set("sync", true);
        this.logDebug(1, msg);
        process.exit(1);
      }
  ...
  applyConfigOverrides() {
    ...
    var env_regex = new RegExp(
      "^" + this.__name.replace(/\W+/g, "_").toUpperCase() + "_(.+)$"
    );
    ...
    if (this.config.get("config_overrides_file")) {
      ...
      try {
        ...
        this.logDebug(
          3,
          "Config overrides file not found, skipping: " + this.coFile
        );
      }
      if (this.coModTime) {
        ...
        try {
          this.configOverrides = JSON.parse(
            fs.readFileSync(this.coFile, "utf8")
          );
        } catch (err) {
          this.logError(
            "config",
            "Config overrides file could not be loaded, skipping: " +
              this.coFile +
              ": " +
              err
          );
  ...
  tick() {
    ...
    if (
      this.coModTime &&
      dargs.epoch - this.coCheckTime >= this.config.freq / 1000
    ) {
      ...
      fs.stat(this.coFile, function(err, stats) {
        ...
        if (mod && mod != self.coModTime) {
          ...
          self.logDebug(
            3,
            "Config overrides file has changed on disk, scheduling reload: " +
              self.coFile
          );
          self.config.mod = 0;
          self.config.check();
  ...
  shutdown(callback) {
    ...
    if (this.config.get("pid_file")) {
      try {
        ...
        this.logDebug(
          9,
          "Deleting PID File: " + this.config.get("pid_file") + ": " + pid
        );
